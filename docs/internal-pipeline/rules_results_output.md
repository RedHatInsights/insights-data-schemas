...
# Format of output files generated by Rules uploader service

Rules Uploader service listens the Insights rules results generated by the
Rules Processing service into the `XXX-insights-rules-results` (where `XXX`
needs to be changed to `prod` etc.) Kafka topic and uploads them to DataHub
(Ceph) bucket.
